...
public class Viewpackage extends JFrame implements ActionListener {
	...
	JButton back;
	JLabel secondusername,checkpack,tftotalperson,tfprice,tfphone,tfdocid,tfdocnumber;
	
	Viewpackage(String username){
		setIconImage(Toolkit.getDefaultToolkit().getImage(Viewpackage.class.getResource("/Gallery/icon1.png")));
		setBounds(430,200,790,480);
		getContentPane().setBackground(Color.black);
		setLayout(null);
		setResizable(false);
		
		
		JLabel title=new JLabel("View Package");
		title.setBounds(140, 25, 350, 30);
		...
		checkpack=new JLabel();
		checkpack.setFont(new Font("Times New Roman",Font.PLAIN,18));
		checkpack.setBounds(250,125,150,25);
		checkpack.setForeground(Color.red);
		add(checkpack);
		...
		tftotalperson=new JLabel();
		tftotalperson.setFont(new Font("Times New Roman",Font.PLAIN,18));
		tftotalperson.setBounds(250,175,150,25);
		tftotalperson.setForeground(Color.red);
		add(tftotalperson);
		...
		ResultSet rs=c.s.executeQuery("Select * from bookpackage where username= '"+username+"'");
		 while(rs.next()) {
			 ...
			 checkpack.setText(rs.getString("selectpackage"));
			 tftotalperson.setText(rs.getString("totalpersons"));
		...
		ImageIcon i4 = new ImageIcon(Viewpackage.class.getResource("/Gallery/viewpack2.jpg"));
		Image i5=i4.getImage().getScaledInstance(380,530,Image.SCALE_DEFAULT);
		ImageIcon i6=new ImageIcon(i5);
	    JLabel image=new JLabel(i6);
		image.setBounds(410,0,380,530);
        add(image);
